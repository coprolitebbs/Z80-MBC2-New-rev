...
char *doAutoExecute(char *atCmd) {
  switch ( atCmd[0] ) {
    case '?':
      ++atCmd;
      out_i2c_ln(settings.autoExecute);
...
char *doI2Cprint(char *atCmd) {
  Serial.print("INBuffer[");
  Serial.print(i2cinbuflen, DEC);
  Serial.print("]: ");
  for (int i = 0; i < i2cinbuflen; i++) {
    Serial.print(' ');
    if ((byte)i2cinbuf[i] < 32) {    
      Serial.print("[");
      Serial.print(i2cinbuf[i],DEC);
      Serial.print("]");
    } else {
      Serial.print(i2cinbuf[i]);
    }
  }
  Serial.println();
  Serial.print("OUTBuffer[");
  Serial.print(i2coutbuflen, DEC);
  Serial.print("]: ");
  for (int i = 0; i < i2coutbuflen; i++) {
    Serial.print(' ');
    if ((byte)i2coutbuf[i] < 32) {    
      Serial.print("[");
      Serial.print(i2coutbuf[i],DEC);
      Serial.print("]");
    } else {
      Serial.print(i2coutbuf[i]);
    }
  }
  Serial.println();
...
char *doWiFiPassword(char *atCmd) {
  switch ( atCmd[0] ) {
    case '?':
      ++atCmd;
      out_i2c_ln(settings.wifiPassword);
...
char *doStartupWait(char *atCmd) {
  switch ( atCmd[0] ) {
    case '?':
      ++atCmd;
      sprintf(emas, "%s\r\n", settings.startupWait == true ? "true" : "false");
      out_i2c(emas);
