...
char *wifiConnection(char *atCmd) {
  ...
  switch ( atCmd[0] ) {
    case '?':
      ++atCmd;
      AddToi2cOutBuff(WiFi.status() == WL_CONNECTED ? '1' : '0');
      out_i2c(endl);
    ...
    case '1':
      ...
      if ( settings.ssid[0] && settings.wifiPassword[0] ) {
        if ( !settings.quiet && settings.extendedCodes ) {
          out_i2c_str("CONNECTING TO SSID ");
          out_i2c(settings.ssid);
        ...
        if ( !settings.quiet && settings.extendedCodes ) {
          out_i2c_str("Congigure SSID and password. Type AT? for help.");
...
//
// ATSGEThttp://host[/path]: fetch a web page
//
// NOTE: http only: no https support
//
char *httpsGet(char *atCmd) {
  char *host, *path, *port;
  //char tmpp[9] = {'h','t','t','p','s',':','/','/','\0'}; //"https://";
  uint16_t portNum;
  //char mdd = 0;
  secure_connected = false;
  //String hts = "https://";
  String allpth = String(atCmd);

  if (allpth.indexOf("http://") > -1) {
    port = strrchr(atCmd, ':');
    ...
    String hst;
    uint16_t strt = allpth.indexOf("https://");


    if (strt > -1) {
      if (strt > 0) {
        allpth = allpth.substring(strt);
      }
    } else {
      allpth = "https://" + allpth;
    }

    strt = allpth.indexOf("https://");
    hst = allpth.substring(strt + 8);

    /*strt = hst.indexOf(":");
    if (strt > -1) {
      String prtt = hst.substring(strt + 1);
      portNum = prtt.toInt();
      hst = hst.substring(0, strt);
      allpth = allpth.substring(0, strt + 8);
    } else*/
      portNum = HTTPS_PORT;

    //allpth = "https://lenta.ru/rss/top7";
    ...
    String allc = "GET " + allpth + " HTTP/1.0\r\nHost: " + hst + "\r\nConnection: close\r\n\r\n";

    char host2[hst.length() + 1];
    hst.toCharArray(host2, hst.length() + 1);


    httpsClient.setInsecure();
    if ( !httpsClient.connect(host2, portNum) ) {
      sendResult(R_NO_CARRIER_SSL);
      //digitalWrite(DCD, !ACTIVE);
    } else {
      connectTime = millis();
      sendResult(R_CONNECT_SSL);
      //digitalWrite(DCD, ACTIVE);
      state = ONLINE;
      secure_connected = true;

      bytesOut += httpsClient.print(allc);
...
char *showNetworkInfo(char *atCmd) {
  ...
  do {		// a Q&D hack to allow ^C to terminate the output at the
    ...
    snprintf_P(infoLine, sizeof infoLine, PSTR("Baud.......: %u"), settings.serialSpeed);
    ...
    snprintf_P(infoLine, sizeof infoLine, PSTR("Bytes in...: %u"), bytesIn);
    if ( PagedOut(infoLine) ) break;
    //snprintf_P(infoLine, sizeof infoLine, PSTR("Bytes out..: %lu"), bytesOut);
    snprintf_P(infoLine, sizeof infoLine, PSTR("Bytes out..: %u"), bytesOut);
    if ( PagedOut(infoLine) ) break;
    //snprintf_P(infoLine, sizeof infoLine, PSTR("Heap free..: %lu"), ESP.getFreeHeap());
    snprintf_P(infoLine, sizeof infoLine, PSTR("Heap free..: %u"), ESP.getFreeHeap());
    if ( PagedOut(infoLine) ) break;
    //snprintf_P(infoLine, sizeof infoLine, PSTR("Sketch size: %lu"), ESP.getSketchSize());
    snprintf_P(infoLine, sizeof infoLine, PSTR("Sketch size: %u"), ESP.getSketchSize());
    if ( PagedOut(infoLine) ) break;
...
char *doQuiet(char *atCmd) {
  switch ( atCmd[0] ) {
    case '?':
      ++atCmd;
      sprintf(emas, "%s\r\n", settings.quiet == true ? "true" : "false");
      out_i2c(emas);
...
char *doVerbose(char* atCmd) {
  switch ( atCmd[0] ) {
    case '?':
      ++atCmd;
      sprintf(emas, "%s\r\n", settings.verbose == true ? "true" : "false");
      out_i2c(emas);
...
char *doExtended(char *atCmd) {
  switch ( atCmd[0] ) {
    case '?':
      ++atCmd;
      sprintf(emas, "%s\r\n", settings.extendedCodes == true ? "true" : "false");
      out_i2c(emas);
